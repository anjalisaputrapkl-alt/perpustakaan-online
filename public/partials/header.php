  ...
  <div class="nav-container">
    <div class="nav-left">
      <a class="brand" href="<?php echo $base; ?>/" title="Kembali ke beranda">
        <svg class="brand-logo" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor"
          stroke-width="2">
      ...
      <nav class="nav-links">
        ...
        <a class="nav-link<?php echo _is_active($base . '/books.php', $current); ?>"
          href="<?php echo $base; ?>/books.php">📖 Buku</a>
        <a class="nav-link<?php echo _is_active($base . '/members.php', $current); ?>"
    ...
    <div class="nav-right">
      <div class="search-box">
        <input type="search" placeholder="Cari buku..." aria-label="Cari buku" data-requires-auth="true" />
...
<script>
  (function () {
    ...
    // Validasi autentikasi untuk searchbox
    var searchInput = document.querySelector('.search-box input[data-requires-auth]');
    if (searchInput) {
      searchInput.addEventListener('focus', function (e) {
        // Cek apakah user sudah login dengan AJAX
        fetch('/perpustakaan-online/public/api/check-auth.php')
          .then(response => response.json())
          .then(data => {
            if (!data.authenticated) {
              // Jika belum login, arahkan ke login.php
              e.target.blur();
              alert('Anda harus login terlebih dahulu untuk menggunakan fitur pencarian');
              window.location.href = '/perpustakaan-online/public/login.php';
            }
          })
          .catch(err => {
